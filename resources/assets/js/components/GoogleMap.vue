    ...
    export default {
        ...
        created: function() {
            ...
            if(!this.currentCenter) {
                ...
                } else if(this.googleMapMode == 'address'){
                    restaurantList = this.$session.get('GPSData');
                }

                if(restaurantList.length == 0) {
                    this.googleMapCenter = {'lat': 39.6622958, 'lng': 128.1452445}
                    return;
                }
                var avg_lat = 0;
                var avg_lng = 0;
                ...
                for(var key in restaurantList) {
                    ...
                    var coordinateLat = Number(restaurantList[key].coordinate_lat);
                    var coordinateLng = Number(restaurantList[key].coordinate_lang);
                    avg_lat += Number(coordinateLat);
                    avg_lng += Number(coordinateLng);
                    count++;
                    console.log(avg_lat);
                    console.log(avg_lng);
